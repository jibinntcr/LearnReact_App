...
function Result({ setCdec, setLoadingCombined }) {
  ...
  const getData = async () => {
    ...
    try {
      const colRef = collection(
        db,
        `/users/${user.uid}/enrolledCourses/cdec/cdecCourses/`
      )
  ...
  const fetchSingleDoc = async (batch, scheduleCourseDocid) => {
    if (
      ...
      const docRef = doc(
        db,
        `AcademicYear/${batch}/cdec/${scheduleCourseDocid}/students`,
        user.uid
      )
      const docCourseRef = doc(
        db,
        `AcademicYear/${batch}/cdec`,
        scheduleCourseDocid
      )
      const docSnap = await getDoc(docRef)
      const docCourseSnap = await getDoc(docCourseRef)
      // console.log(`AcademicYear/${batch}/cdec/${scheduleCourseDocid}`)
  ...
  return (
    ...
    //       <h1 style={{ fontSize: 50 }}> CDeC Result</h1>
    //       <div
    //         style={{
    //           display: "flex",
    ...
    //           </Link>
    //           /
    //           <Link className="hover:text-green-500 ml-1" to="/courses">
    //             CDec Result
    //           </Link>
    //         </div>
    //       </div>
