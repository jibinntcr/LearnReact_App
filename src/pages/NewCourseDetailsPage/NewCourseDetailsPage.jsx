...
function NewCourseDetailsPage() {
  ...
  const { user, enrolledCourses, userDetailsIsFull, userDetails } =
    useUserAuth()
  ...
  const fetchdata = async (collectionName, docId, stateName) => {
    ...
    if (docSnap.exists()) {
      ...
      let fee = docSnap.data().CourseFees
      // console.log("fee")
      stateName({
        ...docSnap.data(),
  ...
  const fund = (
    ...
    if (user && enrolledCourses.includes(DocId)) {
      return "Enrolled"
    }
  ...
  const modelYes = () => {
    setOpen(false)
    navigate(`/checkout/CurriculumCourse/${batch}/${id}`)
  }
                  ...
                  <div className="max-w-2xl mx-auto mt-14 sm: lg:max-w-none lg:mt-0 lg:row-end-2 lg:row-span-2 lg:col-span-3">
                    ...
                    {cash && (
                      <div className="mt-3 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
                        ...
                        >
                          {" "}
                          {cash && cash === undefined ? "Enroll" : cash}
                        </button>
                      </div>
                    )}
                  </div>
...
export default NewCourseDetailsPage
