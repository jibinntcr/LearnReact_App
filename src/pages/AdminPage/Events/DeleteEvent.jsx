...
function DeleteEvent() {
  ...
  function formatEvents(doc) {
    const { title, imageLocation, thumbnailImageLocation } = doc.data()

    return { DocId: doc.id, title, imageLocation, thumbnailImageLocation }
  }

  const DeleteConfirm = (e, title, imageLocation, thumbnailImageLocation) => {
      ...
      buttons: [
        {
          label: "Yes",
          onClick: () =>
            onClickDelete(docId, imageLocation, thumbnailImageLocation),
        },
        {
          label: "No",
          onClick: () => {
  ...
  //event delete
  const onClickDelete = async (docId, fileName, thumbnailImageLocation) => {
    window.scroll(0, 0)

    // console.log(fileName)

    // Create a reference to the image to delete
    const storageRef = ref(storage, fileName, thumbnailImageLocation)
      ...
      {loading ? (
        ...
        <div className="p-5 mt-10  ">
          ...
          <h1 className="text-xl font-serif font-semibold mb-6">
            {" "}
            Total no of events: {data.length}
          </h1>
