...
function GradingCdec() {
  ...
  const fetchData = async () => {
    ...
    const colRef = collection(
      db,
      `/AcademicYear/${batch}/cdec/${courseScheduleId}/students`
    )
  ...
  const submit = async () => {
    ...
    for (let i = 0; i < arr1.length; i++) {
      ...
      // ${remark1} !== ${remark2}`)
      if (
        (grade1 !== grade2 || gradestatus1) !== gradestatus2 ||
        gradeponint1 !== gradeponint2 ||
        mark1 !== mark2 ||
        credit1 !== credit2 ||
        remark1 !== remark2
        ...
        const docRef = doc(
          db,
          `/AcademicYear/${batch}/cdec/${courseScheduleId}/students/${secondObj.DocId}`
        )
