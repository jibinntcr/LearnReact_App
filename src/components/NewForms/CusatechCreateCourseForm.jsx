...
function CusatechCreateCourseForm() {
  ...
  const [courseFacultys, setCourseFacultys] = useState({ value: "", label: "" }) //s    selected faculty data
  ...
  let text = ` Boolean(CourseCode) ${Boolean(
    CourseCode
  )} && Boolean(CourseName)  ${Boolean(CourseName)} 
         && Boolean(CourseFee)  ${Boolean(
           CourseFees >= 0
         )} &&  Boolean(CourseDescription)${Boolean(CourseDescription)}
          &&  Boolean(CourseFaculty)${Boolean(
            courseFacultys.value !== ""
          )} &&  Boolean(CourseDuration)${Boolean(CourseDuration)}
           &&  Boolean(CourseDepartment)${Boolean(
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await addDoc(
        collection(db, "courses/CUSATECH/cusatechChildren"),
        formDataCopy
      )
      setLoading(false)

      toast.success(" Course Created Successfully")
      navigate("/cusatech/admin/CUSATECH/Course/AllCourses")
    } catch (error) {
      setLoading(false)
              ...
              {departmentsList && (
                <>
                  <span className="text-red-600 mr-1">
                    {" "}
                    No Professors Found ⛔️
                  </span>
                  you can add Professors from
                  <Link to="cusatech/admin/Faculty/addFaculty">
                    {" "}
...
export default CusatechCreateCourseForm
