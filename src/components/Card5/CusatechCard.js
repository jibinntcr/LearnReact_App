...
import { MdPeople, MdAccessTimeFilled, MdMoreVert } from "react-icons/md"
import {
  // FaChalkboardTeacher,
  FaSlackHash
} from "react-icons/fa"
...
function CusatechCard({ urlLink, CourseNameInSingleLine, NoOfStd, DocId, CourseCode, Image, CourseName, CourseDepartment, CourseDescription, CourseFees, CourseFaculty, CourseDuration, CourseImage }) {
  ...
  const docRef = doc(db, "courses/CUSATECH/cusatechChildren", DocId)

  //   const CUSATECH = doc(db, "courses/CUSATECH/cusatechChildren",DocId );
      ...
      <div className="p-2  " >
        ...
        <div className="w-60 p-2 bg-white rounded-xl transform transition-all hover:-translate-y-2 duration-300 shadow-lg hover:shadow-2xl">
          ...
          <Link className=" m-2" to={urlLink} >
            <img className="h-40 object-cover rounded-xl" src={CourseImage} alt="" />
          </Link >
          <div className="p-2">
            <div className=" ">
              {/* <!-- Heading --> */}
