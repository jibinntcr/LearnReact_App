...
import Card5 from "../Card5/CourseCard"
...
function CdecHomePageCoursesBox({ setCdecCourseData, setCdecLoading }) {
  // State
  ...
  const fetchCollectionSize = async () => {
    ...
    // Fetch the count of documents in the collection query
  ...
  const fetchDocs = async (countPara) => {
    ...
    if (countPara < remainingPage) {
      ...
      setRemainingPage(PageLimit.CdecHomeCoursePageLimit)
      ...
      if (lastVisible !== "") {
        ...
        q = query(
          ...
          orderBy("scheduleAt", "desc"),
          startAfter(lastVisible),
          limit(PageLimit.CdecHomeCoursePageLimit)
        )
      } else {
...
export default CdecHomePageCoursesBox
